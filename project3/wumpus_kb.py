...
def axiom_generator_percept_sentence(t, tvec):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    states = ['Stench', 'Breeze', 'Glitter', 'Bump', 'Scream']
    op = ''
    if tvec[0] == False:
        op = '~'        #Add negation before false states
    axiom_str += op + states[0] + str(t)
    for i in range(len(tvec)-1):
        op = ''
        if tvec[i+1] == False:
            op = '~'    #Add negation before false states
        axiom_str += ' & ' + op + states[i+1] + str(t)
...
def axiom_generator_initial_location_assertions(x, y):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '(~{0}) & (~{1})'.format(wumpus_str(x, y), pit_str(x, y))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()    
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '{0} <=> ('.format(breeze_str(x, y))
    if x != xmin:
        axiom_str += '({0}) | '.format(pit_str(x - 1, y))
    if x != xmax:
        axiom_str += '({0}) | '.format(pit_str(x + 1, y))
    if y != ymin:
        axiom_str += '({0}) | '.format(pit_str(x, y - 1))
    if y != ymax:
        axiom_str += '({0}) | '.format(pit_str(x, y + 1))
    axiom_str += '({0})'.format(pit_str(x, y))
    axiom_str += ')'    
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '{0} <=> ('.format(stench_str(x, y))
    if x != xmin:
        axiom_str += '({0}) | '.format(wumpus_str(x - 1, y))
    if x != xmax:
        axiom_str += '({0}) | '.format(wumpus_str(x + 1, y))
    if y != ymin:
        axiom_str += '({0}) | '.format(wumpus_str(x, y - 1))
    if y != ymax:
        axiom_str += '({0}) | '.format(wumpus_str(x, y + 1))
    axiom_str += '({0})'.format(wumpus_str(x, y))
    axiom_str += ')'   
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    for x1 in range(xmin, xmax + 1):
        for y1 in range(ymin, ymax + 1):
            axiom_str += '({0} ==> ('.format(wumpus_str(x1, y1))
            for x2 in range(xmin, xmax + 1):
                for y2 in range(ymin, ymax + 1):
                    if x1 != x2 or y1 != y2:    #Locations that are not (x1, y1)
                        axiom_str += '(~{0}) & '.format(wumpus_str(x2, y2))
            axiom_str = axiom_str[:-3]
            axiom_str += ')) & '
    axiom_str = axiom_str[:-3]
    # Comment or delete the next line once this function has been implemented.
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    for x in range(xmin, xmax + 1):
        for y in range(ymin, ymax + 1):
            if x != xi or y != yi:  #Locations that are not (xi, yi)
                  axiom_str += '(~{0}) & '.format(state_loc_str(x, y, t))
    axiom_str = axiom_str[:-3]
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()    
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    directions = ['~' + state_heading_north_str(t), '~' + state_heading_south_str(t), '~' + state_heading_east_str(t), '~' + state_heading_west_str(t)]
    headings = ['north', 'south', 'east', 'west']
    heading_state = directions[headings.index(heading)] 
    directions.remove(heading_state)
    heading_state = heading_state[1:]   # remove '~' from heading state
    axiom_str += '(' + heading_state + ') & (' + ') & ('.join(directions) + ')'
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
...
def axiom_generator_location_OK(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '({0}) <=> ('.format(state_OK_str(x, y, t))
    axiom_str += '~{0} & (~{1} | ~{2}))'.format(pit_str(x, y), state_wumpus_alive_str(t), wumpus_str(x, y))        
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '({0} <=> {1}) | (~{2})'.format(percept_breeze_str(t), breeze_str(x, y), state_loc_str(x,y,t))  
    # If breeze is percieved at t and breeze is present at (x, y) and agent is at (x, y) at time t then it is true
    # If breeze is not percieved at t and breeze is not present at (x, y) and agent is not at (x, y) at time t then it is true
...
def axiom_generator_stench_percept_and_location_property(x, y, t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str = '({0} <=> {1}) | (~{2})'.format(percept_stench_str(t), stench_str(x, y), state_loc_str(x, y, t))
    # If stench is percieved at t and stench is present at (x, y) and agent is at (x, y) at time t then it is true
    # If stench is not percieved at t and stench is not present at (x, y) and agent is not at (x, y) at time t then it is true
    # Assertion is always true
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '({0}) <=> ('.format(state_loc_str(x, y, t + 1))
    axiom_str += '({0} & (~{1} | {2})) | '.format(state_loc_str(x, y, t), action_forward_str(t), percept_bump_str(t + 1))
    if x > xmin:
        axiom_str += '({0} & ({1} & {2})) | '.format(state_loc_str(x - 1, y, t), action_forward_str(t), state_heading_east_str(t))
    if x < xmax:
        axiom_str += '({0} & ({1} & {2})) | '.format(state_loc_str(x + 1, y, t), action_forward_str(t), state_heading_west_str(t))
    if y > ymin:
        axiom_str += '({0} & ({1} & {2})) | '.format(state_loc_str(x, y - 1, t), action_forward_str(t), state_heading_north_str(t))
    if y < ymax:
        axiom_str += '({0} & ({1} & {2})) | '.format(state_loc_str(x, y + 1, t), action_forward_str(t), state_heading_south_str(t))
    axiom_str = axiom_str[:-3]
    axiom_str += ')'
...
def axiom_generator_heading_north_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '({0}) <=> ('.format(state_heading_north_str(t + 1))
    axiom_str += '({0} & ~{1} & ~{2}) | '.format(state_heading_north_str(t), action_turn_left_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}) | '.format(state_heading_west_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}))'.format(state_heading_east_str(t), action_turn_left_str(t))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
...
def axiom_generator_heading_east_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '({0}) <=> ('.format(state_heading_east_str(t + 1))
    axiom_str += '({0} & ~{1} & ~{2}) | '.format(state_heading_east_str(t), action_turn_left_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}) | '.format(state_heading_north_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}))'.format(state_heading_south_str(t), action_turn_left_str(t))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
...
def axiom_generator_heading_south_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '({0}) <=> ('.format(state_heading_south_str(t + 1))
    axiom_str += '({0} & ~{1} & ~{2}) | '.format(state_heading_south_str(t), action_turn_left_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}) | '.format(state_heading_east_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}))'.format(state_heading_west_str(t), action_turn_left_str(t))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
...
def axiom_generator_heading_west_ssa(t):
    ...
    axiom_str = ''
    "*** YOUR CODE HERE ***"
    axiom_str += '({0}) <=> ('.format(state_heading_west_str(t + 1))
    axiom_str += '({0} & ~{1} & ~{2}) | '.format(state_heading_west_str(t), action_turn_left_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}) | '.format(state_heading_south_str(t), action_turn_right_str(t))
    axiom_str += '({0} & {1}))'.format(state_heading_north_str(t), action_turn_left_str(t))
    # Comment or delete the next line once this function has been implemented.
    #utils.print_not_implemented()
...
def axiom_generator_only_one_action_axioms(t):
    ...
    for action_do in actions:
        axiom_str += '(' + action_do + ' <=> ('
        for action_dont in actions:
            if action_do != action_dont:
                axiom_str += '~' + action_dont + ' & '
        axiom_str = axiom_str[:-3]
        axiom_str += ')) & '
    axiom_str = axiom_str[:-3]
    # Comment or delete the next line once this function has been implemented.
