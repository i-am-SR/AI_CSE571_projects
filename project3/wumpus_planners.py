...
class PlanRouteProblem(search.Problem):
    ...
    def h(self,node):
        ...
        The manhattan distance to the nearest goal state
        """
        "*** YOUR CODE HERE ***"
    ...
    def actions(self, state):
        ...
        res = ['TurnRight', 'TurnLeft']     # Allowed motion actions from every state
        if ((state[0] + 1, state[1]) in self.allowed and state[2] == directions.index('east')) or ((state[0] - 1, state[1]) in self.allowed and state[2] == directions.index('west')) or ((state[0], state[1] + 1) in self.allowed and state[2] == directions.index('north')) or ((state[0], state[1] - 1) in self.allowed and state[2] == directions.index('south')):
            res.append('Forward')       # If the next state in the forward direction is alllowed, add forward to the list of valid actions
        return res
        #pass
    ...
    def result(self, state, action):
        ...
        directions = ['north', 'west', 'south', 'east']
        if action == 'Forward':
            if state[2] == directions.index('north'):
                return (state[0], state[1] + 1, state[2])
            if state[2] == directions.index('south'):
                return (state[0], state[1] - 1, state[2])
            if state[2] == directions.index('east'):
                return (state[0] + 1, state[1], state[2])
            if state[2] == directions.index('west'):
                return (state[0] - 1, state[1], state[2])
    ...
    def goal_test(self, state):
        ...
        if (state[0], state[1]) in self.goals:
            return True     #If the current state is one if the goal states, return true
        else:
            return False
...
class PlanShotProblem(search.Problem):
    ...
    def h(self,node):
        """
        Heuristic that will be used by search.astar_search():
        The manhattan distance to the nearest allowed state that has the x or y coordinate same as that of a possible wumpus location
        """
        "*** YOUR CODE HERE ***"
        locations = []
        for (x, y) in self.allowed:
            for (x_wumpus, y_wumpus) in self.goals:
                if x == x_wumpus or y == y_wumpus:
                    locations.append((x, y))

        return min([manhattan_distance_with_heading(node.state, goal) for goal in locations])
        #pass
    ...
    def goal_test(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Return true if the current state has the x or y coordinate (but not both) same as that of a possible wumpus location
        directions = ['north', 'west', 'south', 'east']
        for (x_wumpus, y_wumpus) in self.goals:
            if state[0] == x_wumpus and state[1] != y_wumpus:
                if (y_wumpus > state[1] and state[2] == directions.index('north')) or (y_wumpus < state[1] and state[2] == directions.index('south')):
                    return True
            elif state[1] == y_wumpus and state[0] != x_wumpus:
                if (x_wumpus > state[0] and state[2] == directions.index('east')) or (x_wumpus < state[0] and state[2] == directions.index('west')):
                    return True
        return False
